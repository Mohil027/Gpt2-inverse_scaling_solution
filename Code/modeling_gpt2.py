...
class SeqAttention(nn.Module):
    ...
    def __init__(self, hidden_size, nb_heads, attn_span, dropout, **adapt_span_params):
        nn.Module.__init__(self)
        self.dropout = nn.Dropout(dropout)
        self.hidden_size = hidden_size  # size of a single head
        self.attn_span = attn_span
        self.adaptive_span = AdaptiveSpan(attn_span=attn_span, nb_heads=nb_heads, **adapt_span_params)
...
class GPT2Attention(nn.Module):
    def __init__(self, config, is_cross_attention=False, layer_idx=None):
        ...
        self.attn = SeqAttention(
            ...
            adapt_span_ramp=config.max_position_embeddings // 4,
            adapt_span_init=0.5,
            adapt_span_cache=True,
        )
        self.proj_query = nn.Linear(self.embed_dim, self.embed_dim, bias=False)
        self.proj_out = nn.Linear(self.embed_dim, self.embed_dim, bias=False)
        self.proj_val = nn.Linear(self.embed_dim, self.embed_dim, bias=False)
        self.proj_key = nn.Linear(self.embed_dim, self.embed_dim, bias=False)
